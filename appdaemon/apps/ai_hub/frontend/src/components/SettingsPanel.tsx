 ...
 *
 * On save, calls POST /api/appdaemon/ai_hub/settings/save.
 * A restart reminder is shown after saving LLM-related settings.
 */
...
const KEY_FIELDS: KeyField[] = [
  ...
  { key: 'appdaemon_token',    label: 'HA Long-Lived Token', placeholder: 'eyJ...' },
]
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Section title="API Keys">
        <p className="text-xs text-ha-muted -mt-2">
          Keys are written to <code className="text-ha-blue">/config/secrets.yaml</code> on the HA device.
        ...
        <div className="flex flex-col gap-3">
          {KEY_FIELDS.map(field => {
            const isConfigured = settings.keys_configured[
              field.provider ?? field.key.replace('_api_key', '').replace('github_pat', 'github_pat').replace('appdaemon_token', 'appdaemon_token')
            ] ?? false
      ...
      <div className="flex items-center gap-4 pt-1">
        ...
        <p className="text-xs text-ha-muted">
          Restart the AppDaemon add-on after saving to apply LLM provider changes.
        </p>
      </div>
    </div>
  )
